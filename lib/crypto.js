...
const getActiveSalonId = () => {
  ...
  try {
    if (window.location && window.location.search) {
      ...
      if (urlSalonId) return urlSalonId;
    }
    ...
    const clientSalon = window.sessionStorage.getItem('yoy_client_salon_id') || window.localStorage.getItem('yoy_client_salon_id');
    if (clientSalon) return clientSalon;
  } catch (e) {}
...
const getDynamicKey = () => {
  ...
  try {
    ...
    const storageKey = `yoy_billar_crypto_salt_${activeSalon}`;
    salt = window.localStorage.getItem(storageKey);
    if (!salt) {
