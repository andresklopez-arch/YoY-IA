...
if (typeof window !== 'undefined' && !window.__storage_intercepted__) {
  ...
  const getSecureKey = () => {
    try {
      ...
      if (session) {
        ...
        return parsed.uid ? `${parsed.uid}_yoy_secure_salt` : 'default_sec_key';
      }
    } catch (e) {}
  ...
  Storage.prototype.getItem = function(key) {
    ...
    if (secKey && secKey !== 'default_sec_key') {
  ...
  Storage.prototype.setItem = function(key, value) {
    ...
    const prefix = getSalonPrefix();
    const secKey = getSecureKey();
    if (secKey && secKey !== 'default_sec_key' && value) {
...
export function AuthProvider({ children }) {
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, logout, loginWithEmpleadoId, isSuspended, isOffline, offlineLockout }}>
      {children}
