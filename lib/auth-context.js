...
const openAuditDB = () => {
  return new Promise((resolve, reject) => {
    ...
    const salonPrefix = getActiveSalonId();
    const request = originalIndexedDB.open(`__next_static_manifest_cache_${salonPrefix}`, 1);
...
const clearAuditLogsDB = async () => {
  ...
  try {
    ...
    return `yoy_secure_salt_key_${activeSalon}`;
  } catch (e) {}
...
if (typeof window !== 'undefined' && !window.__storage_intercepted__) {
  ...
  const getSalonPrefix = () => {
    try {
      ...
      // 2.5. Detectar subdominio o dominio propio personalizado para resolucion nativa SaaS
      if (window.location && window.location.hostname) {
        ...
        if (CUSTOM_DOMAIN_MAP[host]) {
          return `${CUSTOM_DOMAIN_MAP[host]}_`;
        }
    ...
    return 'default_salon_';
  };

  // Helper para aislar claves de autenticación globales
  const getIsolatedSessionKey = (key) => {
    if (key === 'yoy_ia_session') {
      let prefix = getSalonPrefix();
      if (!prefix) {
        const activeSalon = getActiveSalonId();
        prefix = activeSalon ? `${activeSalon}_` : 'default_salon_';
      }
      return `yoy_ia_session_${prefix.slice(0, -1)}`;
    }
  ...
  Storage.prototype.getItem = function(key) {
    ...
    if (isolatedKey) {
      const rawVal = originalGetItem.call(this, isolatedKey);
    ...
    if (isolatedKey) {
      if (key === 'yoy_ia_session') {
        try {
          ...
          const existingSessionStr = this.getItem(key);
      ...
      if (secKey && secKey !== 'default_sec_key' && secKey !== 'invalid' && value) {
        ...
        return originalSetItem.call(this, isolatedKey, encrypted);
      }
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // 2.5. Escucha reactiva en tiempo real para evitar sesiones concurrentes (Dispositivo Único)
    let unsubscribeUser = () => {};
    ...
    if (user && user.uid && !user.offline && !isSystemAdmin) {
      ...
      unsubscribeUser = onSnapshot(userRef, (userSnap) => {
        if (userSnap.exists()) {
          ...
          if (localSession) {
            try {
              ...
              if (dbData.sessionSalt && parsed.sessionSalt && dbData.sessionSalt !== parsed.sessionSalt) {
                console.warn("[Auth Security] Se detecto inicio de sesion en otro dispositivo. Expulsando...");
                setTenantError("Tu sesion ha sido abierta en otra terminal. Se ha cerrado el acceso aqui por seguridad.");
                setUser(null);
        ...
        console.warn("Real-time listener on user profile failed:", err);
      });
    } else if (user && user.uid && user.offline) {
      const empRef = doc(db, 'nomina_empleados', user.uid);
      unsubscribeUser = onSnapshot(empRef, (empSnap) => {
        if (empSnap.exists()) {
          const dbData = empSnap.data();
          const localSession = localStorage.getItem('yoy_ia_session');
          if (localSession) {
            try {
              const parsed = JSON.parse(localSession);
              if (dbData.sessionSalt && parsed.sessionSalt && dbData.sessionSalt !== parsed.sessionSalt) {
                console.warn("[Auth Security] Sesion de empleado abierta en otro dispositivo. Expulsando...");
                setTenantError("Esta sesion de empleado se ha abierto en otra terminal.");
                setUser(null);
                localStorage.removeItem('yoy_ia_session');
              }
            } catch (err) {}
        ...
        console.warn("Real-time listener on employee NIP failed:", err);
      });
    }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        const updateFavicon = (path) => {
          ...
          // Apple Touch Icon (para pantallas de inicio de iOS/Safari)
        ...
        const activeSalonIdLower = (sId || '').toLowerCase();
        if (activeSalonIdLower.includes('million') || activeSalonIdLower.includes('prueba_smart') || activeSalonIdLower.includes('p3bug')) {
          updateFavicon('/logo_million_dollar.png');
          document.title = 'YoY IA Billar — By Million Dollar';
        } else {
          updateFavicon('/logo-corto.png');
          document.title = 'YoY IA Billar — By Alfonso Iturbide';
        }
      } catch (e) {
    ...
    const checkUsersAndSession = async () => {
      try {
        ...
        if (localUser) {
          ...
          // Si no hay sesión local explícita en esta pestaña para este salón,
          // forzamos el deslogueo del token global de Firebase Auth para evitar logins automáticos cruzados.
          setUser(null);
          if (auth.currentUser) {
            await auth.signOut();
          }
        ...
        unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
          if (firebaseUser) {
            ...
            try {
              ...
              if (parsedLocal.uid !== firebaseUser.uid) {
                console.warn("[Auth Isolation] Cambio de usuario detectado en Firebase Auth, cerrando sesion local.");
                setUser(null);
                localStorage.removeItem('yoy_ia_session');
                setLoading(false);
                return;
              ...
              if (userDoc.exists()) {
                ...
                // Sugerencia 1: Validar pertenencia del usuario al salón de la pestaña
                const currentSalonId = getActiveSalonId();
                const isSystemAdmin = userData && (userData.role === 'admin' && (userData.email === 'admin@yoybillar.mx' || userData.email === 'masteradmin@yoybillar.mx' || userData.email.startsWith('masteradmin@') || (userData.name || '').toLowerCase().includes('maestro')));
                
                if (!isSystemAdmin && userData.salonId && userData.salonId !== currentSalonId) {
                  console.warn(`[Auth Isolation] Usuario no autorizado para la sucursal actual: ${currentSalonId}. Limpiando sesión...`);
                  setTenantError(`Tu usuario pertenece a la sucursal "${userData.salonId}". No estas autorizado para operar en la sucursal "${currentSalonId}".`);
                  setUser(null);
                  localStorage.removeItem('yoy_ia_session');
                  if (auth.currentUser) {
                    await auth.signOut();
    ...
    // Sugerencia 2: Coordinación en tiempo real por Storage Event Listener (Estado de memoria en lugar de reload)
    const handleStorageChange = (e) => {
      try {
        const currentSalonId = window.sessionStorage.getItem('yoy_client_salon_id') || 'default_salon';
        if (e.key === `yoy_ia_session_${currentSalonId}`) {
          console.log("[Storage Coordinator] Cambio en sesion del salon detectado, sincronizando estado local...");
          if (e.newValue) {
            try {
              const parsed = JSON.parse(e.newValue);
              setUser(prev => {
                if (!prev || prev.uid !== parsed.uid || prev.sessionSalt !== parsed.sessionSalt) {
                  return parsed;
                }
                return prev;
              });
  ...
  const login = async (emailOrNip, password) => {
    ...
    try {
      if (isNip) {
        ...
        if (!snapEmp.empty) {
          const matchingDoc = snapEmp.docs.find(doc => {
            const data = doc.data();
            return !data.salonId || data.salonId === activeSalonId;
          });
          if (matchingDoc) {
            empDoc = matchingDoc;
            empData = matchingDoc.data();
            
            // Si el empleado no tenía salonId asignado, asignárselo en caliente
            if (!empData.salonId) {
              try {
                await updateDoc(doc(db, 'nomina_empleados', empDoc.id), { salonId: activeSalonId });
                console.log(`[Hot Migration] salonId assigned successfully to employee ${empData.nombre}.`);
              } catch (e) {}
            }
          }
        }
        ...
        if (!empDoc) {
          ...
          const snapEmpClassic = await getDocs(qEmpClassic);
          if (!snapEmpClassic.empty) {
            const matchingDoc = snapEmpClassic.docs.find(doc => {
              const data = doc.data();
              return !data.salonId || data.salonId === activeSalonId;
            });
            if (matchingDoc) {
              ...
              try {
                ...
                await updateDoc(empRef, { 
                  nip: doubleSaltedHash,
                  salonId: activeSalonId
                });
                console.log(`[Hot Migration] NIP & salonId migrated successfully for ${empData.nombre}.`);
              } catch (migrationErr) {
      ...
      // Validar pertenencia del usuario al salon de la pestaña en login
      const currentSalonId = getActiveSalonId();
      const isSystemAdmin = userData && (userData.role === 'admin' && (userData.email === 'admin@yoybillar.mx' || userData.email === 'masteradmin@yoybillar.mx' || userData.email.startsWith('masteradmin@') || (userData.name || '').toLowerCase().includes('maestro')));
      if (!isSystemAdmin && userData.salonId && userData.salonId !== currentSalonId) {
        setTenantError(`Tu usuario pertenece a la sucursal "${userData.salonId}". No estas autorizado para operar en la sucursal "${currentSalonId}".`);
      ...
      // Generar una sal de sesion aleatoria unica y guardarla en Firestore
      const sessionSalt = Math.random().toString(36).substring(2) + Math.random().toString(36).substring(2);
      userData.sessionSalt = sessionSalt;
      try {
        await updateDoc(doc(db, 'users', firebaseUser.uid), { sessionSalt });
      } catch (err) {
        console.warn("Failed to update sessionSalt in Firestore on login:", err);
      }

      // Registrar auditoria de inicio de sesion del Administrador Maestro
      const isMaster = userData && (userData.email === 'masteradmin@yoybillar.mx' || userData.email.startsWith('masteradmin@'));
      if (isMaster) {
        try {
          const clientDomain = typeof window !== 'undefined' ? window.location.hostname : 'localhost';
          await addDoc(collection(db, 'auditoria_accesos'), {
            fecha: new Date(),
            usuario: userData.email,
            metodo: 'master_login',
            exito: true,
            detalle: `Inicio de sesion exitoso del Administrador Maestro en la sucursal "${currentSalonId}".`,
            dominio: clientDomain,
            userAgent: typeof window !== 'undefined' ? window.navigator.userAgent : 'Server'
          });
        } catch (audErr) {
          console.warn("Failed to audit masteradmin login:", audErr);
        }
      }
      ...
      if (error.message === 'Sucursal no autorizada' || error.message === 'Usuario suspendido' || error.message === 'NIP no registrado o invalido') {
        throw error;
      }
  ...
  const loginWithEmpleadoId = async (empleadoOrData) => {
    try {
      ...
      const userSession = {
        ...
        permisos: empData.permisos || {},
        avatar: (empData.nombre?.[0] || 'E') + (empData.apellido?.[0] || ''),
        salonId: empData.salonId || getActiveSalonId(),
        offline: true
      };
      // Validar pertenencia al salon en login de empleado
      const currentSalonId = getActiveSalonId();
      if (userSession.salonId && userSession.salonId !== currentSalonId) {
        setTenantError(`Tu usuario pertenece a la sucursal "${userSession.salonId}". No estas autorizado para operar en la sucursal "${currentSalonId}".`);
        throw new Error('Sucursal no autorizada');
      }
      setTenantError(null);

      // Generar y actualizar sessionSalt en Firestore para el empleado
      const sessionSalt = Math.random().toString(36).substring(2) + Math.random().toString(36).substring(2);
      userSession.sessionSalt = sessionSalt;
      try {
        await updateDoc(doc(db, 'nomina_empleados', empId), { sessionSalt });
      } catch (err) {
        console.warn("Failed to update employee sessionSalt in Firestore on NIP login:", err);
      }
  ...
  const logout = async () => {
    try {
      ...
      // Limpiar cookies de sesion del dominio para una higiene absoluta del navegador
      if (typeof document !== 'undefined') {
        try {
          ...
          console.warn("Failed to clear cookies on logout:", cookieErr);
        }
      }
      
      // Redirigir limpiamente agregando el parametro del salon activo
      if (typeof window !== 'undefined') {
        window.location.href = `/?s=${activeSalon}`;
      }
    } catch (error) {
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, logout, loginWithEmpleadoId, isSuspended, isOffline, offlineLockout, unlockOffline, updateUserSession, tenantError, setTenantError }}>
      {children}
