...
import { doc, getDoc, collection, getDocs, limit, query, where, addDoc, updateDoc, serverTimestamp, onSnapshot } from 'firebase/firestore';
...
const openAuditDB = () => {
  return new Promise((resolve, reject) => {
    ...
    const request = indexedDB.open('yoy_secure_db', 1);
    request.onupgradeneeded = (e) => {
      const db = e.target.result;
      if (!db.objectStoreNames.contains('audit_logs')) {
        db.createObjectStore('audit_logs');
      }
    };
    request.onsuccess = (e) => {
...
export function AuthProvider({ children }) {
  ...
  const logOfflineUnlock = async (supervisorName, supervisorId) => {
    try {
      ...
      // Migración desde LocalStorage a IndexedDB
      ...
      if (oldPayload) {
        let parsedVal = oldPayload;
        try {
          parsedVal = JSON.parse(oldPayload);
        } catch (e) {}
        await setAuditLogsDB(parsedVal);
        removeFn.call(localStorage, 'yoy_offline_audit_log');
        removeFn.call(localStorage, 'yoy_sys_state_cache');
        console.log("[IndexedDB Migration] Migrated offline logs from localStorage to IndexedDB.");
      }
      
      // Obtener logs desde IndexedDB
      ...
      if (dbPayload) {
        try {
          ...
          if (parsedPayload.integrity !== expectedIntegrity) {
            wasCorrupted = true;
            await triggerTamperAlert("Firma de integridad del bloque corrupta en IndexedDB.");
            logs = [];
          } else {
            ...
            if (decryptedStr) {
              logs = JSON.parse(decryptedStr);
              if (!verifyHashChain(logs)) {
                wasCorrupted = true;
                await triggerTamperAlert("Eslabón de hash roto en la cadena de auditoría offline.");
                logs = [];
              }
            } else {
              wasCorrupted = true;
              await triggerTamperAlert("Fallo de descifrado en los datos de la bitácora offline.");
              logs = [];
            }
          }
        } catch (decErr) {
          console.warn("Fallo al procesar logs locales desde IndexedDB:", decErr);
          wasCorrupted = true;
          await triggerTamperAlert(`Fallo de parseo/descifrado: ${decErr.message}`);
          logs = [];
        }
      }
      ...
      if (wasCorrupted) {
        ...
        tamperLog.hash = calculateLogHash(tamperLog, tamperPrevHash);
        logs.push(tamperLog);
      }
      ...
      newLog.hash = calculateLogHash(newLog, prevHash);
      logs.push(newLog);
      ...
      if (secKey && secKey !== 'default_sec_key' && secKey !== 'invalid') {
        ...
        payloadToWrite = {
          data: encrypted,
          integrity: generateChecksum({ data: encrypted }, 'yoy_audit_log_salt'),
          truncated: truncated
        };
      } else {
        payloadToWrite = {
          data: serialized,
          integrity: generateChecksum({ data: serialized }, 'yoy_audit_log_salt'),
          truncated: truncated
        };
      }
      
      await setAuditLogsDB(payloadToWrite);
      console.log(`[Offline Audit] Desbloqueo registrado en IndexedDB, encadenado y firmado para ${supervisorName}.`);
    } catch (err) {
      ...
      try {
        ...
        if (decryptedStr) {
          ...
          if (!verifyHashChain(logs)) {
            ...
            await triggerTamperAlert("Sincronización abortada por eslabón de hash roto.");
            await clearAuditLogsDB();
            return;
          }
        } else {
          console.error("[Audit Sync] Decryption failed! Dropping logs.");
          await triggerTamperAlert("Sincronización abortada por fallo de descifrado.");
          await clearAuditLogsDB();
          return;
        }
      } catch (decErr) {
      ...
      try {
        ...
        if (phone) {
          const text = wasTruncated
            ...
            : `ℹ️ *Auditoría Offline*\nSe detectó reconexión. Se subieron exitosamente ${logs.length} logs de desbloqueo offline acumulados durante el periodo de desconexión (Ledger Validado).`;
