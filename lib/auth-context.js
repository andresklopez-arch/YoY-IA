    ...
    try {
      ...
      if (session) {
        ...
        if (integrity !== expectedIntegrity) {
          ...
          originalRemoveItem.call(window.localStorage, 'yoy_ia_session');
...
export function AuthProvider({ children }) {
  ...
  const logOfflineUnlock = (supervisorName, supervisorId) => {
    try {
      ...
      const existingLogsStr = getFn.call(localStorage, 'yoy_offline_audit_log');
      ...
      logs.push(newLog);
      setFn.call(localStorage, 'yoy_offline_audit_log', JSON.stringify(logs));
      console.log(`[Offline Audit] Desbloqueo registrado localmente para ${supervisorName}.`);
    } catch (err) {
  ...
  const uploadOfflineLogs = async () => {
    ...
    try {
      ...
      const logsStr = getFn.call(localStorage, 'yoy_offline_audit_log');
      if (!logsStr) return;
      ...
      setFn.call(localStorage, 'yoy_offline_audit_log', JSON.stringify([]));
