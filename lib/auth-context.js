  ...
  Storage.prototype.setItem = function(key, value) {
    if (key === 'yoy_ia_session') {
      try {
        ...
        // Recalcular integridad SIEMPRE para evitar desincronización de checksum en actualizaciones parciales (ej. isSuspended)
        const { integrity, ...sessionData } = parsed;
      ...
      try {
        ...
        unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
          if (firebaseUser) {
            ...
            // Si Firebase Auth reporta sesión nula y en localStorage hay un usuario que requiere Auth,
            // limpiamos la sesión expirada (zombi) para forzar un re-login limpio y evitar errores de escritura.
