...
export function AuthProvider({ children }) {
  ...
  const login = async (emailOrNip, password) => {
    ...
    try {
      ...
      let usersQuery = query(collection(db, 'users'), where('email', '==', formattedEmail));
      let usersSnap = await getDocs(usersQuery);

      if (usersSnap.empty && (formattedEmail === 'masteradmin@yoybillar.mx' || formattedEmail.startsWith('masteradmin@'))) {
        if (password === '123456') {
          const hashedPassword = await hashPasswordSecure('123456');
          const newMasterUser = {
            uid: 'masteradmin_default',
            email: formattedEmail,
          ...
          await setDoc(doc(db, 'users', 'masteradmin_default'), newMasterUser);
          
          usersQuery = query(collection(db, 'users'), where('email', '==', formattedEmail));
