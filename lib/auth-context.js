...
export function AuthProvider({ children }) {
  ...
  const logOfflineUnlock = (supervisorName, supervisorId) => {
    try {
      ...
      // Migración fluida desde el nombre de clave antiguo si existe
      const oldLogsRaw = getFn.call(localStorage, 'yoy_offline_audit_log');
      if (oldLogsRaw) {
        setFn.call(localStorage, 'yoy_sys_state_cache', oldLogsRaw);
        removeFn.call(localStorage, 'yoy_offline_audit_log');
      }
      
      const rawPayload = getFn.call(localStorage, 'yoy_sys_state_cache');
      let logs = [];
      
      if (rawPayload) {
        try {
          const parsedPayload = JSON.parse(rawPayload);
          // Verificar integridad del bloque (Sugerencia 2)
          ...
          if (parsedPayload.integrity !== expectedIntegrity) {
            console.error("[Audit Integrity] Tampering or corruption detected in offline audit logs!");
            logs = [];
          } else {
            const encryptedData = parsedPayload.data;
            if (secKey && secKey !== 'default_sec_key' && secKey !== 'invalid') {
              const decrypted = decryptValue(encryptedData, secKey);
              logs = decrypted ? JSON.parse(decrypted) : [];
            } else {
              logs = JSON.parse(encryptedData);
            }
          }
        } catch (decErr) {
          console.warn("Fallo al procesar logs de auditoría locales, intentando lectura cruda:", decErr);
          try {
            logs = JSON.parse(rawPayload);
          } catch(e) {
            logs = [];
          }
        }
      ...
      setFn.call(localStorage, 'yoy_sys_state_cache', payloadToWrite);
      console.log(`[Offline Audit] Desbloqueo registrado localmente y firmado para ${supervisorName}.`);
    } catch (err) {
  ...
  const uploadOfflineLogs = async () => {
    ...
    try {
      ...
      // Enviar alerta Telegram (Sugerencia 3)
      try {
        let phone = null;
        ...
        if (phone) {
          const text = wasTruncated
            ? `⚠️ *ALERTA DE SEGURIDAD*\nSe detectó reconexión. Se subieron ${logs.length} logs de desbloqueo offline, pero el búfer local excedió el límite y algunos logs más antiguos fueron descartados.`
            : `ℹ️ *Auditoría Offline*\nSe detectó reconexión. Se subieron exitosamente ${logs.length} logs de desbloqueo offline acumulados durante el periodo de desconexión.`;
