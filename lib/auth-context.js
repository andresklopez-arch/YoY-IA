...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const checkUsersAndSession = async () => {
      try {
        ...
        // 2. Fallback a Firebase Auth si no hay localUser
  ...
  const login = async (emailOrNip, password) => {
    ...
    try {
      ...
      // 1. Intentar iniciar sesión a través de la API del servidor (evita problemas de permisos en cliente)
      try {
        ...
        if (res.ok && data.success && data.customToken) {
          ...
          const userSession = {
            ...
            role: data.user.role || data.user.rol || 'usuario',
            alias: data.user.alias || formattedEmail.split('@')[0],
            salonId: data.user.salonId || 'default_salon',
            ...data.user
          };
          ...
          console.warn("Fallo en API de login del servidor. Intentando fallback a Firebase Auth clásico:", data.error);
        }
      } catch (apiErr) {
        if (apiErr.message === 'Credenciales incorrectas' || apiErr.message === 'Contraseña incorrecta' || apiErr.message === 'Usuario no encontrado' || apiErr.message.includes('incorrecta')) {
          throw apiErr;
        }
        console.warn("Error de conexión al API de login. Intentando fallback clásico:", apiErr);
      }

      // 2. Fallback clásico a Firebase Auth (útil en entornos locales sin admin SDK)
