...
const getActiveSalonId = () => {
  ...
  try {
    ...
    // 2. Detectar subdominio personalizado para resolucion nativa SaaS (ej. pruebasmart.yoybillar.mx)
    ...
    // Si entramos a la URL raíz sin parámetros ni subdominio, purgamos sessionStorage
    if (window.location && (!window.location.search || !(new URLSearchParams(window.location.search).has('s') || new URLSearchParams(window.location.search).has('salonId')))) {
      sessionStorage.removeItem('yoy_client_salon_id');
      return 'default_salon';
    }

    // 3. Verificar si la pestaña tiene un salonId guardado en su sessionStorage aislado
