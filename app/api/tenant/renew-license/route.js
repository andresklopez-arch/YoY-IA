...
try {
  ...
  console.warn("Firebase Admin SDK no se pudo inicializar en renew-license:", e.message);
}
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      if (masterTgDoc.exists) {
        ...
        if (tgData.enabled && tgData.botToken && tgData.chatId) {
          const messageText = `♻️ *[ALR SaaS] Licencia Renovada*\n\n` +
                              ...
                              `• *Nombre:* ${nombre}\n` +
                              `• *Embajador:* ${embajador}\n` +
                              `• *Licencia:* \`${licData.numeroLicencia}\`\n` +
                              `• *Nuevo Vencimiento:* ${vencimientoIso.split('T')[0]}\n` +
                              `• *Renovado por:* ${decodedToken.email}`;
      ...
      console.warn("Fallo al enviar alerta de renovación a Telegram:", tgErr.message);
    }
